...
async def main_handler(event: Message):
    ...
    if msg == "Старт" or msg == "Начать":
        ...
        conn = sq.connect("teachers_db.sqlite")
        cur = conn.cursor()
        cur.execute(f'''SELECT reminder FROM teachers_table WHERE teacher_vk_id = "{id_user}"''')
        try:
        ...
        if remind:
            cur.execute(f'''UPDATE teachers_table SET reminder = 0 WHERE teacher_vk_id = "{id_user}"''')
            conn.commit()
            conn.close()
            await event.answer(message=f"Напоминалка выключена", keyboard=settings_keyboard_false)
        else:
            cur.execute(f'''UPDATE teachers_table SET reminder = 1 WHERE teacher_vk_id = "{id_user}"''')
            conn.commit()
            conn.close()
